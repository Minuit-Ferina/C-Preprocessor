...
// Get the next "..." string
String.prototype.getNextStringInclude = function() {
	var str = this.match(/^<([A-Za-z0-9\-_\. \/\\]+)>/);
	return (!str || !str[1]) ? '' : str[1];
};
...
var Compiler = function(opt) {
	...
	this.options.basePath = new Array();
	this.options.stopOnError = true;
	this.options.enumInHex = true;

	this.errors_array = new Array();
...
Compiler.prototype._error = function(text) {
	this.errors_array.push(text);
};


// Emit a success
Compiler.prototype._success = function(code) {
	this.emit('success', {"code": code, "errors": this.errors_array});
};
...
Processor.prototype.error = function(msg) {
	let _error = new Object;

	_error.file = this.currentFile;
	_error.path = this.path;
	_error.line = this.currentLine;
	_error.message = msg;

  	this.parent._error(_error);
...
createDirective("include", function(text) {
	...
	if (fs.existsSync("./" + file))
		path = "./"
	else if (fs.existsSync("./" + this.path + file))
		path = "./" + this.path
	else
		for (let i of this.options.basePath) {
			if (fs.existsSync(i + file))
				path = i;
			else if (fs.existsSync(i + this.path + file))
				path = i + this.path;
			// console.log(i)
		}
	...
	fs.readFile(path + file, 'utf8', function(err, code) {
		...
		if (err) {
			_this.error(`include file not found "${file}"`);
			return _this.next();
